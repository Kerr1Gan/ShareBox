...
public class EasyServer extends Thread implements HTTPRequestListener {
        ...
        if (mType == TYPE_AP) {
            ...
            if (mListener != null) {
                if (!hsl.getHTTPServer(0).isOpened() || hsl.getHTTPServer(0).getServerSock().isClosed()
                        || FileUtil.port == 0)
                    mListener.ready(this, "", 0);
                else
                    mListener.ready(this, getBindIP(), getHTTPPort());
    ...
    public void httpRequestReceived(HTTPRequest httpReq) {
        ...
        String reqMethod = "";
        String localUri = uri.toLowerCase();
        if (localUri.startsWith(CONTENT_API_URI)) {
            try {
            ...
            return;
        } else if (localUri.startsWith(CONTENT_FILE_URI)) {
...
//        BaseServlet html=new Html();
    ...
    private HostInterface.ICallback mListener = null;

    /**
     * only if listener invoked , the listener will be erased.
     */
    public void setServerListener(HostInterface.ICallback listener) {
