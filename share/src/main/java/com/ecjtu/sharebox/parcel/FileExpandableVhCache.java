...
public class FileExpandableVhCache extends ParcelableFileCacheHelper {

    public FileExpandableVhCache(String path) {
        super(path);
    }
    ...
    protected <T> T readParcel(Parcel parcel) {
        ...
        List<FileExpandableAdapter.VH> ret = new ArrayList<>();
        for (int i = 0; i < size; i++) {
            ...
            FileExpandableAdapter.VH child = new FileExpandableAdapter.VH(group, childList);
    ...
    protected <T> Parcel writeParcel(Parcel parcel, T object) {
        if (!(object instanceof List) || ((List) object).size() <= 0 || !(((List) object).get(0) instanceof FileExpandableAdapter.VH)) {
            return null;
        }
        List list = (List) object;
        parcel.writeInt(list.size());
        for (int i = 0; i < list.size(); i++) {
            FileExpandableAdapter.VH vh = (FileExpandableAdapter.VH) list.get(i);
            List<String> childList = vh.childList;
            parcel.writeStringList(childList);
            parcel.writeString(vh.group);
